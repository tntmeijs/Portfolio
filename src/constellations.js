...
/**
 * Create all datapoints needed to render a constellation shaped like a lightbulb
 * @param {number} x Constellation's top-left corner's horizontal position
 * @param {number} y Constellation's top-left corner's vertical position
 * @param {number} width Contellation's bottom-right corner's horizontal position
 * @param {number} height Constellation's bottom-right corner's vertical position
 * @returns {import("./stars").ConstellationData} Constellation data
 */
export const getLightbulbConstellationData = (x, y, width, height) => (
  {
    coordinates: [
      // Bulb
      { x: x + (width * 0.35), y: y + (height * 0.8) },
      { x: x + (width * 0.3), y: y + (height * 0.65) },
      { x: x + (width * 0.1), y: y + (height * 0.5) },
      { x: x, y: y + (height * 0.35) },
      { x: x + (width * 0.05), y: y + (height * 0.175) },
      { x: x + (width * 0.25), y: y + (height * 0.05) },
      { x: x + (width * 0.5), y: y },
      { x: x + (width * 0.75), y: y + (height * 0.05) },
      { x: x + (width * 0.95), y: y + (height * 0.175) },
      { x: x + width, y: y + (height * 0.35) },
      { x: x + (width * 0.9), y: y + (height * 0.5) },
      { x: x + (width * 0.7), y: y + (height * 0.65) },
      { x: x + (width * 0.65), y: y + (height * 0.8) },

      // Cap
      { x: x + (width * 0.35), y: y + (height * 0.875) },
      { x: x + (width * 0.65), y: y + (height * 0.85) },
      { x: x + (width * 0.35), y: y + (height * 0.9375) },
      { x: x + (width * 0.65), y: y + (height * 0.9125) },
      { x: x + (width * 0.35), y: y + height },
      { x: x + (width * 0.65), y: y + (height * 0.975) },

      // Wire
      {x: x + (width * 0.45), y: y + (height * 0.75)},
      {x: x + (width * 0.35), y: y + (height * 0.45)},
      {x: x + (width * 0.65), y: y + (height * 0.45)},
      {x: x + (width * 0.55), y: y + (height * 0.75)}
    ],
    edges: [
      // Bulb
      { startIndex: 0, endIndex: 1 },
